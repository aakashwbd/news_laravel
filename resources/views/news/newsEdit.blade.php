    ...
    <form id="editNewsform" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group margin-top-40" id="newsType">
            ...
            <select name="news_type" id="news_type" class="form-control create-form">
                <option @if($target->news_type == "image") ? selected : @endif value="image">Image</option>
                <option @if($target->news_type == "video") ? selected : @endif value="video">Video</option>
            </select>
        ...
        <div class="form-group margin-top-40" id="video_type">
            ...
            <select name="video_type" id="video_type" class="form-control create-form">
                <option @if($target->video_type == "youtube") ? selected :  @endif value="youtube">Youtube</option>
                <option @if($target->video_type == "dailymotion") ? selected :  @endif value="dailymotion">DailyMotion</option>
                <option @if($target->video_type == "vimeo") ? selected :  @endif value="vimeo">Vimeo</option>
                <option @if($target->video_type == "m3u8") ? selected :  @endif value="m3u8">M3u8</option>
                <option @if($target->video_type == "mp4") ? selected :  @endif value="mp4">Mp4</option>
            </select>
            {{-- <input name="name" type="text" class="form-control create-form" id="name" placeholder="Category Name"> --}}
        </div>
        ...
        <div class="form-group margin-top-40" style="position: relative">
            ...
            <select name="category_type" id="category_type" class="form-control create-form">
                <option @if($target->catgegory_type == "non-feature") ? selected : @endif value="non-feature">Non-feature</option>
                <option @if($target->category_type == "feature") ? selected : @endif value="feature">Feature</option>
            </select>
            {{-- <input name="name" type="text" class="form-control create-form" id="name" placeholder="Category Name"> --}}
        </div>
        <div class="form-group">
...
<script>
    ...
    $(function () {
        ...
        $(document).on("click", "#editNews", function (e) {
            ...
            var url = "{{ URL::to('admin/news/update') }}"
