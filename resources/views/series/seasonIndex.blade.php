    ...
    <div class="content-heading">
        <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <span class="sub-title">
                            <a href="/series/season" class="title-btn red" id="subCategory">Season</a>
                        </span>
            ...
            <div class="col-md-4 text-right">
                <form action="/series/season/filter" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="col-md-4 single-content margin-top-40">
                        ...
                        <div class="single-content-wraper margin-top-20">
                            ...
                            <div class="total-videos margin-top-20">
                                ...
                                <span>Total Videos This Season</span>
                            </div>
                            <div class="content-actions margin-top-20">
                                <form action="{{ URL::to('series/' . $data->id . '/Season') }}" method="POST">
                                    @csrf
                                    @method('DELETE')

                                    <button title="Edit" type="button" data-toggle="modal" data-target="#createModal"
                                        id="seasonEditBtn" data-id="{{ $data->id }}">
                                        <span class="iconify" data-icon="ant-design:edit-filled"></span>
                                        Edit Season
                                    </button>

                                    <button type="submit" onclick="return confirm('Are you sure?')" title="Delete" id="Season">
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $(document).on("click", "#seasonCreate", function(e) {
                e.preventDefault();
                $.ajax({
                    url: "{{ URL::to('series/season-create') }}",
                    type: "POST",
                    dataType: "json",
                    headers: {
            ...
            $(document).on("click", "#createSeason", function(e) {
                ...
                $.ajax({
                    url: "{{ URL::to('series/season-store') }}",
                    type: 'POST',
                    dataType: "json",
                    processData: false,
                    contentType: false,
            ...
            $(document).on("click", "#editSeason", function(e) {
                ...
                $.ajax({
                    url: "{{ URL::to('series/season-update') }}",
                    type: 'POST',
                    dataType: "json",
                    processData: false,
                    contentType: false,
